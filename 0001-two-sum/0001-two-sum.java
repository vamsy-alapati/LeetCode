...
//     }
    ...
    public int[] twoSum(int[] nums, int target) {
        ...
        for (int i=0; i<nums.length; i++) {
            ...
            // since we insert first and only then we find the complement, 
            // we reverse the order of returning the array to get correct output.
            if (map.containsKey(complement)) {
                return new int[] { map.get(complement), i};
            }
            
            map.put(nums[i],i);  //insert element of array into map until we find complement.
            
        }
        return null;
    }
    
    //Similar Implementation with List - but poor performance
//     public int[] twoSum(int[] nums, int target) {
